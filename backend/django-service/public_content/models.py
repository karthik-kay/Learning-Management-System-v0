...
class SuccessStory(PublishableModel):
    ...
    track = models.CharField(max_length=120, db_index=True)
    background = models.CharField(max_length=120, db_index=True)
    outcome_type = models.CharField(max_length=120, db_index=True)
    ...
    salary_before_lpa = models.DecimalField(
        max_digits=6, decimal_places=2, null=True, blank=True
    )
    salary_after_lpa = models.DecimalField(
        max_digits=6, decimal_places=2, null=True, blank=True
    )
    salary_jump_percent = models.PositiveIntegerField(default=0)
    time_to_hire_months = models.PositiveIntegerField(null=True, blank=True)
    is_placed = models.BooleanField(default=True, db_index=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["status", "is_featured"]),
            models.Index(fields=["track", "background", "outcome_type"]),
            models.Index(fields=["slug"]),
        ]

    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = build_unique_slug(self, source_field="headline")
        super().save(*args, **kwargs)

    def __str__(self):
        return f"{self.student_name} — {self.after_title}"
