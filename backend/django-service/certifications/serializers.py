...
class PublicCertificationListSerializer(serializers.ModelSerializer):
    type = serializers.CharField(source="get_certification_type_display")
    domain_label = serializers.CharField(source="get_domain_display")

    class Meta:
        model = CertificationOffering
        fields = [
            "id",
            "slug",
            "title",
...
class PublicCertificationDetailSerializer(PublicCertificationListSerializer):
    related_course_slug = serializers.CharField(
        source="course.slug", allow_null=True, read_only=True
    )
    related_program_slug = serializers.CharField(
        source="program.slug", allow_null=True, read_only=True
    )

    class Meta(PublicCertificationListSerializer.Meta):
        fields = PublicCertificationListSerializer.Meta.fields + [
            "description",
            "outcomes",
            "requirements",
            "assessment_methods",
